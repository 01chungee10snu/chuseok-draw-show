...
## D020 — 자연스러운 속도와 보간 (v1.1, D012 대체)

초반 fast-forward를 제거하고 물리 속도는 최대 1배로 유지한다. 10ms 고정 스텝 사이의 위치·각도를 보간해 감속 구간의 계단식 움직임을 없앤다. 감속은 마지막 5 world units에서 0.58배까지, 줌은 1.6배까지 적용한다. 카메라 x는 코스 중앙에 고정하고 y/zoom은 연속적으로 움직인다. 경사로 끝은 벽 안쪽까지 이어 붙이고 마지막 경사로는 중앙 출구를 향하도록 한다. 기본 최소 18초, 여유 최소 24초에 맞추며 긴 경로를 억지로 빠르게 재생하지 않는다.

## D021 — 골인 대상과 진출 대상의 일치 (D009/D015의 연출 입력 규칙 갱신)

인원비례 drawRound 또는 균등 drawSubset은 이전과 같이 먼저 한 번 확정한다. 익명·동일 크기 공으로 Box2D의 실제 궤적과 결승선 통과 시점을 계산하고, 확정 진출자를 앞선 골인 경로에 경기 시작 전에 한 번 배정한다. 나머지 공은 나머지 경로에 배정한다. 이름·그룹 인원수·추첨 결과는 경로 생성에 입력되지 않고 경기 중 라벨 교체, 순간이동, 경로 수정은 없다. 공정 추첨과 물리 경로의 연결 방식은 앱 진행 방식과 README에 공개한다.

그룹 라운드는 A/B 편당 대표 공 한 개로, 먼저 골인한 편의 모든 고유값 그룹이 진출한다. 결선은 개인 공으로 먼저 골인한 k개가 진출한다. 물리 재생 완료 시 arrivedIds와 advancingIds 집합이 정확히 일치해야 다음 라운드를 적용한다. 골인 표시를 1.4초 유지한다. 과거의 시간 제한만으로 물리 골인처럼 종료하는 분기는 제거한다. 궤적 준비는 60초분의 시뮬레이션으로 제한하며 실패 시 정적 공개라고 명시하고 확정 결과를 유지한다. 동작 줄이기와 비물리 게임에는 골인했다고 주장하지 않는다. 재시도에도 확정 추첨 결과와 경로 seed를 보존한다.

## D022 — 시작 대상의 헤더 고유값 다중 선택

대기 화면에서 직접 대상 선택으로 진입한다. 한 헤더 안의 선택 값은 OR로 결합하고, 미입력은 빈 문자열 범주로 처리한다. 빈 선택은 0명이며 전체 참가자로 바꾸지 않는다. 인원 미리보기에는 이전 당첨자 제외 설정도 반영한다. 필터 적용 이후의 실제 모수가 개인 확률 계산의 분모이다. 진행 중 필터 변경에는 기존 초기화 확인을 유지하며 명단이나 선택 값의 새 영구 저장은 추가하지 않는다.

## D023 — 실제 스프레드시트 호환성과 보수적인 헤더 판정 (v1.2)

파일은 `File.text()`에 맡기지 않고 원시 바이트에서 UTF-8을 먼저 엄격히 검사한 뒤 CP949(EUC-KR)로 fallback한다. BOM이 있으면 UTF-16 LE/BE를 사용한다. 구분자는 쉼표·탭·세미콜론 또는 Excel `sep=` 지시문에서 명확하게 결정하고, 행 열 수·따옴표·중복 ID 오류는 계속 전체 가져오기를 거부한다. 한 컬럼 이름 명단은 구분자가 없어도 허용한다.

이름/ID는 NFKC, 공백, 대소문자와 `_`·`-`를 정규화한 정확한 별칭으로만 찾는다. 전체 이메일·전화번호처럼 민감하거나 자유 서술일 가능성이 높은 헤더는 계속 차단한다. 이메일 첫 글자는 글자 또는 숫자 한 자, 전화번호 끝 한 자리는 숫자 한 자일 때만 예외로 허용한다.

범주가 2~7개이면 기존처럼 사용한다. 8~100개는 참가자 수를 범주 수로 나눈 평균이 2 이상일 때만 허용해 거의 고유한 자유 텍스트를 피한다. 한 범주의 참가자는 서로 다른 Lane으로 나누지 않는다. 표준 Excel 오류 리터럴은 선택 컬럼에서 원문 행을 버리지 않고 `(미입력)` 그룹으로 취급하며 헤더와 건수를 경고한다. 이름/ID의 오류는 신원 생성으로 우회하지 않고 가져오기를 거부한다.

저장소에 포함하지 않은 188행 CP949 검수 파일에서 원본 비식별 기준 7개가 모두 사용 가능했고 앱 파생 `성씨 초성`까지 8개가 설정에 표시됐다. 실제 파일이나 참가자 행은 테스트 fixture와 저장소에 넣지 않는다.

## D024 — 자동 진행과 정확한 고정 라운드 일정 (v1.2)

기본값은 데이터와 생존자 수에 맞추는 자동 진행이다. 직접 설정은 요청 게임 수 2~12, 라운드별 12~90초(기본 24초), 13종 중 게임 하나 또는 자동 선택을 저장한다. 참가자가 `N>=2`명이면 실제 게임 수는 `min(요청 수, N-1)`이고 `N<=1`이면 0이다. 모든 게임은 적어도 한 명을 줄이며 마지막 게임은 정확히 한 명을 남긴다. 이름 공개와 진행자의 확인 화면은 게임으로 세지 않는다.

남은 사람이 10명을 넘고 남은 게임이 3개 이상이면 두 Lane 모두 이후 게임 수 이상의 인원을 보존하는 미사용 그룹 기준을 우선한다. 조건을 만족하지 않으면 균등 subset 크기를 남은 게임 수에 맞춰 계산한다. 10명을 넘는 익명 subset은 선택한 물리 코스를 가장하지 않고 라이트 그리드로 명시한다. 10명 이하는 개인 공과 선택한 호환 게임을 사용한다.

직접 설정의 초는 물리 코스 재생 시간이며 골인 확인 약 1.4초는 별도다. 짧은 설정은 이미 계산한 같은 경로의 재생 속도를 높이고, MC가 다음 단계를 누르기까지의 시간은 예상 합계에서 제외한다. 자동 모드는 실제 물리 속도를 높이지 않는 최소 18/24초 정책을 유지한다. 일정이나 대상을 바꾸면 확인 후 세션을 초기화하지만, 일시정지·화면 오류·재생 재시도는 확정 결과와 seed를 유지한다. 참가자 데이터는 설정과 함께 영구 저장하지 않는다.

## D025 — 13종 게임과 접촉 기반 코스 동작 (v1.2, D015 갱신)

자동 shuffle bag은 그룹 게임 10종을 사용한다. 이 중 물리 코스는 기존 5종과 기어 캐스케이드·브레이크어웨이의 7종이고, 오비트 랠리·라이트 그리드·리듬 터널은 Canvas 게임이다. 결선 물리 3종까지 합치면 선택 가능한 13종 중 10종이 Box2D다. 직접 설정 목록은 특정 배열 길이를 복제하지 않고 전체 Stage registry에서 만든다.

새 물리 맵은 회전뿐 아니라 왕복 셔틀·피스톤·공전·스윙 운동을 사용한다. 브레이크어웨이는 공 접촉 뒤 발판 body를 비활성화하고 재생에서도 같은 시점에 사라지게 한다. 이 구성은 참고 소스 commit `47230e3`의 맵 primitive와 접촉 기반 장애물 수명주기에서 영감을 받았으며, 앱 고유의 course registry·고정 경로 재생·Web Crypto 추첨 계약에 맞게 다시 구성했다. 참고 소스의 MIT 고지는 `THIRD_PARTY_NOTICES.md`에 유지한다.
